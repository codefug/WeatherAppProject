...
export default (json)=>{
    ...
    Array.from(afterImage).forEach((element,index)=>{
        ...
        습도 : ${json.list[index+1].main.humidity} \r\n
        최대 온도 : ${json.list[index+1].main.temp_max} \r\n
        최소 온도 : ${json.list[index+1].main.temp_min}`;
        getWeatherIcon(json,afterImage[index],afterh2[index],json.list[index+1].dt_txt.slice(11,13));
